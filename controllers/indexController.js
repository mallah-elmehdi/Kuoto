...
exports.filter = catchAsync(async (req, res, next) => {
    ...
    if (req.body.ville.toString() != '/^/')
        link =
            link +
            '/' +
            slugify(req.body.ville.join(' '), {
                lower: true,
            });
    if (req.body.secteur.toString() != '/^/')
        link =
            link +
            '/' +
            slugify(req.body.secteur.join(' '), {
                lower: true,
            });
    if (req.body.marque.toString() != '/^/')
        link =
            link +
            '/' +
            slugify(req.body.marque.join(' '), {
                lower: true,
            });
    if (req.body.type)
        link =
...
exports.trier = catchAsync(async (req, res, next) => {
    ...
    if (global.ville.toString() != '/^/')
        link =
            link +
            '/' +
            slugify(global.ville.join(' '), {
                lower: true,
            });
    if (global.secteur.toString() != '/^/')
        link =
            link +
            '/' +
            slugify(global.secteur.join(' '), {
                lower: true,
            });
    if (global.marque.toString() != '/^/')
        link =
            link +
            '/' +
            slugify(global.marque.join(' '), {
                lower: true,
            });
    if (global.type)
        link =
