...
exports.carDetails = catchAsync(async (req, res, next) => {
  ...
  const reviews = await review.find({ which: req.params.id }).populate("from", "name");
...
exports.cherche = catchAsync(async (req, res, next) => {
  req.body.chercher = slugify(req.body.chercher, { lower: true });
  ...
  if (req.body.ville.toString() != "/^/") link = link + "/" + slugify(req.body.ville.join(" "), { lower: true });
  if (req.body.secteur.toString() != "/^/") link = link + "/" + slugify(req.body.secteur.join(" "), { lower: true });
  link = link + "/" + slugify(req.body.marque.join(" "), { lower: true });
  link = link + "/" + slugify(req.body.modele.join(" "), { lower: true });
...
exports.trier = catchAsync(async (req, res, next) => {
  var link = "/res";
  if (global.ville.toString() != "/^/") link = link + "/" + slugify(global.ville.join(" "), { lower: true });
  if (global.secteur.toString() != "/^/") link = link + "/" + slugify(global.secteur.join(" "), { lower: true });
  if (global.marque.toString() != "/^/") link = link + "/" + slugify(global.marque.join(" "), { lower: true });
  if (global.type) link = link + "/" + slugify(global.type.join(" "), { lower: true });
  for (var property in req.body) {
...
exports.resend = catchAsync(async (req, res, next) => {
  let usrC = await user.findOne({ email: req.body.email });

  usrC = usrC || (await store.findOne({ email: req.body.email }));
